...
let infoCheck = () => {
    request('http://127.0.0.1:4040/api/tunnels/command_line', (err, response, body) => {
        if (err) {
            ...
            if (ngrok_data.error_code) { 
                console.log(`error ${ngrok_data.msg}`);
                console.log(`please close (Ctrl + C) and type the command again...`)
            };
...
exports.expose = (options) => {
    if (fs.exists(options.ngrokpath, (exist) => {
        exist ? initngrok(options) : console.log(`
        ngrok binary not found!.
        Please use -N option for custom ngrok path.
